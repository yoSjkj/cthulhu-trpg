...
총 43개 항목, 8단계. 각 단계가 끝나면 게임이 동작하는 상태를 유지한다.
...
| # | 작업 |
|---|---|
| 0-1 | `docs/` 문서 6개 배치 (tone, game-rules, architecture, scenario-spec, backlog, CLAUDE.md) |
| 0-2 | `CoC_PWA_기획안.md`에서 룰 수치 섹션 전체 삭제. 설계 의도·게임 흐름만 남긴다 |
| 0-3 | `DEV_LOG.md` 삭제. 코드보다 낡아 정보 가치가 없다 |
| 0-4 | `README.md` 작성 — 소개, 실행법, 문서 지도 |
| 0-5 | 삭제 전 커밋. 기획안의 미확인 맥락 보존 |
| 0-6 | `.gitignore`에 `.env` 계열 확인 |
...
| 2-3 | `useLuck`이 난이도를 무시하고 `skillValue` 기준으로 비용 계산 | `difficulty` 인자 추가. 목표치 기준으로 계산. 대실패·SAN 굴림 사용 차단 |
...
| # | 현상 | 수정 |
|---|---|---|
| 2-9 | `applySanLoss`가 `startSAN` 자리에 현재 SAN을 넘김 — 임계값이 계속 낮아져 부정기 광기가 매 턴 발동 | 세션 시작 SAN을 store에 저장하고 전달 |
...
| 3-3 | `Game.jsx` `CharacterSheet` | 능력치 한글 표기 통일. POW는 '의지', CON은 '체력', SIZ는 '체격'. `ABILITY_MAP`과 일치시킬 것 |
...
| # | 파일 | 작업 |
|---|---|---|
| 4-1 | `scenario-spec` 준수 | 적 스탯 스키마 확정. `maxHP`, `DEX`, `skills`, `weapon` 필수. 기존 시나리오 JSON 갱신 |
| 4-2 | `combat.js` | `performEnemyAttack()` 추가. 적 무기·기술로 판정하고 피해 계산 |
| 4-3 | `Game.jsx` | 전투 라운드 루프 구현. 플레이어 행동 → 적 행동(코드) → 결과를 AI에 전달 |
...
| 6-3 | `keeper.js` | `keeper_notes`를 키 포함해 전달. 현재는 값만 이어붙임 |
| 6-4 | `keeper.js` | 시스템 프롬프트를 정적/동적으로 분리. 정적 부분을 앞에 두어 캐싱 가능하게 |
...
| # | 현상 | 수정 |
|---|---|---|
| 7-1 | `messages`, `historySummary`, `insanityTurnsLeft`, `combatEnemy`, `checkedLocations`가 `useState` — 새로고침하면 AI가 대화 맥락을 전부 잃음 | store로 이동해 persist |
...
| 8-2 | `api/` | 엔드포인트·인증·요청 형식 변경. system을 `messages[0]`으로. 응답은 `choices[0].message.content` |
| 8-3 | `api/` | 모델 슬러그 상수 분리 |
| 8-4 | `keeper.js` | `summarizeHistory`에 타임아웃 추가. 현재 없어서 무한 대기 가능 |
| 8-5 | `keeper.js` | `normalizeKeeperResponse()` — 모든 필드 기본값 채움. 성공·실패 경로 공통. 429/5xx 지수 백오프 재시도 |
